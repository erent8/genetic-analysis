...
def fetch_pdyn_gene(accession_id):
    try:
        ...
        for feature in record.features:
            feature_str = f"Feature Type: {feature.type}\nLocation: {feature.location}"
            gene_data.append((feature_str, ""))
            for key, value in feature.qualifiers.items():
                gene_data.append((f"{key.capitalize()}: ", ", ".join(value)))
...
def create_gui(data):
    ...
    tree = ttk.Treeview(root, columns=("Feature", "Value"), show='headings', height=30)
    tree.heading("Feature", text="Özellik")
    tree.heading("Value", text="Değer")
    
    # Sütun genişliklerini ayarla
    tree.column("Feature", width=300, anchor=tk.W)
